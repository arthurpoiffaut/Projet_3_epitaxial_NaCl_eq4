...
while (flag==1)
    ...
    b=(S./k).*dx^2; 
    %condition x=0
    b(1)=-2*dx^2*(To-T(1))+2*dx*(T(1)^4*si+c3);%%-2*dx^2*(To-T(1))+2*dx*(T(1)^4*si+c3); 
    %conditon x=L
    b(N+1)=-2*dx^2*(To-T(N))+2*dx*(T(N)^4*si-d3);%-2*dx^2*(To-T(N+1))+2*dx*(T(N+1)^4*si+d3);%=2*dx*(T(N+1)^4*si+d3);%-2*dx^2*(To-T(N+1))+%2*dx*(T(N+1)^4*si+d3);%-2*dx*(T(N+1)^4*si+d3);%2*dx^2*(To-T(N+1))-2*dx*(T(N+1)^4*si+d3);%2*d3*dx;
